...
" Id:               $Date: 2015-04-12 12:59:48+09 $
"                   $Revision: 3.19 $
"
...
function s:MorePrevLineIndent(pline, pnum, line, ind)
  ...
  elseif a:pline =~ '\\$' && a:line !~ '\\$'
    let [pline, pnum] = s:JoinContinueLine(a:pline, a:pnum)
...
function s:PrevLineIndent(line, lnum, pline, ind, ...)
  ...
  elseif a:line !~ '^\s*#' && a:line =~ '|\|&\|`\|('
    ...
    if a:0 && line =~ '\\\@<!`' && len(split(a:1['line'], '\\\@<!`', 1)) % 2
      let ind = ind - &sw
    elseif a:0 && line =~ '\\\@<!`'
      let ind = ind + &sw
    endif
...
function s:HideCaseLabelLine(line, pline)
  ...
  if a:line =~ '^\s*[^(].\{-})' && a:line !~ ';;\s*\%(#.*\)\=$'
    ...
    let line = substitute(line, '\\\%o47\|\\\@<!\%o47.\{-}\\\@<!\%o47'
          \. '\|\(\\\@<!\\*"\).\{-}\\\@<!\1', '', 'g')
...
function s:HideAnyItemLine(line, lnum)
  ...
  if line =~ '\%(\${\h\w*#\=\|\${\=\|\\\)\@<!#'
    ...
    let line = substitute(line, '\\\%o47\|\\\@<!\%o47.\{-}\\\@<!\%o47'
          \. '\|\(\\\@<!\\*"\).\{-}\\\@<!\1', '', 'g')
  elseif line =~ "'"
    let line = substitute(line, '\\\%o47\|\\\@<!\%o47.\{-}\\\@<!\%o47', '', 'g')
  endif
  if line =~ '"' && len(split(line, '"', 1)) % 2
...
function s:OnOrNotItem(line, item)
  let line = substitute(a:line, '\\\@<!\%o47.\{-}\\\@<!\%o47'
        \. '\|\(\\\@<!\\*"\).\{-}\\\@<!\1', '', 'g')
...
function s:GetJoinLineAndQuoteInit(lnum)
  ...
  while snum <= a:lnum
    ...
    elseif !qinit && nline =~ '\%o47'
      let nline = substitute(nline, '\\\%o47\|'. "'[^']*'", '', 'g')
    elseif !qinit && nline =~ '"' && len(split(nline, '\\\@<!"', 1)) % 2
    ...
    if !qinit && nline =~ '\\\@<!"\|\\\@<!\%o47'
      ...
      elseif !init1 && init2
        ...
        let qinit = 1
      elseif init1 && !init2
        let line = substitute(line, '\\\%o47\|'. "'[^']*'", '', 'g')
        let qinit = 2
      else
    ...
    elseif qinit == 2 && nline =~ '\\\@<!"'
          \ && len(split(line, '\\\@<!"', 1)) % 2
      let line = substitute(line, '\(\\\@<!\\*"\).\{-}\\\@<!\1', '', 'g')
      let qinit = s:InsideQuoteReverse(fline, qinit)
...
function s:SkipQuoteLine(line, lnum, qinitdic)
  let lnum = a:lnum
  while s:GetPrevNonBlank(lnum) && a:qinitdic[s:prev_lnum]
    let lnum = s:prev_lnum
  endwhile
  unlet! s:prev_lnum
...
function s:InsideQuoteReverse(line, init)
  ...
  while sum > 0
    ...
    if str == '#'
      ...
      let dquote = 0
      break
    elseif str == '\' && (squote == sum + 1 || dquote == sum + 1)
      let squote = 0
      let dquote = 0
